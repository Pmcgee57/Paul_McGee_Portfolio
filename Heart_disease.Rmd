...
```
```{r}
head(hd_data) %>% kbl(caption = "Raw Data") %>% kable_classic(full_width = F, html_font = "Cambria")
```
...
```
```{r, echo=FALSE}
head(hd_data)  %>% kbl(caption = "Recoded Data") %>% kable_classic(full_width = F, html_font = "Cambria")
```
&nbsp;
...
library(broom)
tidy_m <- tidy(model)
tidy_m  %>% kbl(caption = "Coefficients Table") %>% kable_classic(full_width = F, html_font = "Cambria")
```
...
tidy_m  %>% kbl(caption = "Odds Ratio of Coefficients") %>% kable_classic(full_width = F, html_font = "Cambria")
```
