...
```
```{r, echo= FALSE}
head(dat) %>% kbl(caption = "Raw Data") %>% kable_classic(full_width = T, html_font = "Cambria")
#str(dat)
```
...
I'll have a look at a quick summary of the data. Some quick observations can be made such as that the range of ages is between 18 and 64 and the maximum charged for health care cost is $63,770.00
...
sex_plot + xlab("Sex") + ylab("Charges")
```
...
smoke_plot + xlab("Smoker") + ylab("Charges")
```
...
kid_plot + xlab("Number of Children")
```
...
```
```{r,echo=FALSE}
head(dat_2) %>% kbl(caption = "Data with New Variable") %>% kable_classic(full_width = T, html_font = "Cambria")
```
&nbsp;  
